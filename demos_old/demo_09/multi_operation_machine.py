"""
Simulation of multi-operation machine.
...
`MultiStationMachine` sends randomly `True` or `False` to the `LoadingStation`
every few seconds to indicate whether the rotating disk is ready or not.
"""
import logging
import random
...
class MultiOperationMachine(AbstractPLC):
    ...
    def _init_control(self) -> None:
        if self._init_flag:
            ...
            self.X0.activate()
            self.logger.info("multi-operation machine ready.")
    ...
    def exit_routine(self) -> None:
        self.logger.info("Exit multi-operation machine.")
