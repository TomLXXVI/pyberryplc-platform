"""
Loading station PLC program.

Here it is demonstrated how simultaneous tasks can be implemented that run side
by side using the SFC approach.
...
disk of the multi-operation machine. Each item in the batch is moved through three
...
class LoadingStation(AbstractPLC):
    ...
    def _init_control(self):
        if self.init_flag:
            ...
            self.X0.activate()
            self.logger.info("Loading station is ready.")
    ...
    def _sequence_control(self):
        ...
        # Check rotary disk of multi-operation machine is ready to accept a new 
        # item.
        ...
        if self.X40.active and main_exited.active:
            # When main program closes, multi-operation machine may already be
        ...
        if self.X40.active and not self.pos3_free.active and rotarydisk_ready.active:
            # An item is at POS3 and the rotary disk from the multi-operation 
            # machine is ready.
            self.X40.deactivate()
