...
class AutomaticPanel:
    ...
    def __init__(self, parent: 'CNCHMI'):
        ...
        self.trajectory: PointToPointTrajectory | None = None
        self.coords = None
    ...
    def _build_motion_profile_sliders(self):
        with self.ui.column().classes("border rounded p-4 w-full"):
            ...
            self.alpha_max_slider = self.ui.slider(
                ...
                on_change=lambda e: self._on_alpha_max_slider_change()
            )
            ...
            self.omega_max_slider = self.ui.slider(
                min=100.0, max=5000.0, step=100.0, value=1500.0,
                on_change=lambda e: self._on_omega_max_slider_change()
            )
    ...
    def _build_cont_motion_switch(self):
        with self.ui.column().classes("border rounded p-4 w-full"):
            self.cont_motion_switch = self.ui.switch(
                "continuous motion",
                value=False,
                on_change=lambda e: self._on_cont_motion_change()
            )
    ...
    def _send_to_plc(self) -> None:
        if not self.trajectory:
            ...
            return
        self.parent.hmi_data.data["trajectory"] = self.trajectory.get_stepper_signals()
    ...
    def _on_alpha_max_slider_change(self) -> None:
        self._update_contents()

    def _on_omega_max_slider_change(self) -> None:
        self._update_contents()
    ...
    def _on_cont_motion_change(self) -> None:
        self._update_contents()
    ...
    def _compute_trajectory(
        ...
        self.pos = self.trajectory.position_profiles
        self.vel = self.trajectory.velocity_profiles
        self.acc = self.trajectory.acceleration_profiles
    ...
    def _update_contents(self):
        self._compute_trajectory(
            points=self.points,
            alpha_max=(self.alpha_max_slider.value, self.alpha_max_slider.value),
            omega_max=(self.omega_max_slider.value, self.omega_max_slider.value),
