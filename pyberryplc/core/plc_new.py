...
class AbstractPLC(ABC):
    ...
    def run(self, measure: bool = False) -> None | dict:
        ...
        while not self._exit:
            ...
            try:
                ...
                elif self.mode == PLCMode.EMERGENCY:
                    # In emergency stop, we keep repeating the emergency routine
                    # (to keep outputs forcibly low).
                    self.emergency_routine()  # to be implemented in derived class

                    if self._check_reset_condition():
                        self.logger.info("Reset received. Recovering from Emergency...")
                        self._recover_to_initial_state()

            except EmergencyException:
                # First click on the emergency stop: switch PLC mode
                if self.mode != PLCMode.EMERGENCY:
                    self.mode = PLCMode.EMERGENCY
                    self.logger.critical("EmergencyException caught! Switching to EMERGENCY mode.")
                    self.emergency_routine()

            except Exception as error:
                self.mode = PLCMode.FAULT
                self.crash_routine(error)  # to be implemented in derived class
                break  # in the event of an unexpected crash: stop

            finally:
                # Failsafe: Outputs are ALWAYS written, including the 'False'
                # outputs during an emergency stop.
                self._write_outputs()
            ...
            # `True`, not when while-loop is interrupted by `Exception`.
            self.logger.info(
    ...
    def _check_reset_condition(self) -> bool:
        """
        Checks if the conditions for exiting the emergency stop have been met.
        For example: Emergency stop button physically up AND ResetRequest is
        active.
        """
        # 1. If the local physical emergency stop button is still pressed
        # (Low/False), we must NEVER reset in any case.
        if "EmergencyButton" in self.input_register and not self.input_register["EmergencyButton"].active:
            return False

        # 2. Check if this specific PLC detects the Reset Button (physical or HMI) activated
        if "ResetButton" in self.input_register and self.input_register["ResetButton"].active:
            return True
        if "ResetButton" in self.marker_register and self.marker_register["ResetButton"].active:
            return True

        # 3. IF the unit is connected to DB0, the MainPLC can centrally clear the emergency stop
        if hasattr(self, "db0"):
            # If MainPLC has reset the central emergency stop flag,
            # the sub-PLC may also attempt to recover.
            if not self.db0.data["EmergencyStopActive"].active:
                return True

        return False

    def _recover_to_initial_state(self) -> None:
        """
        Returns the PLC to the safe initial state (Grafcet initial steps).
        """
        # 1. Deactivate ALL steps/markers
        for marker in self.marker_register.values():
            marker.update(False)

        # 2. Specifically activate the initial steps (Grafcet initial state).
        # Recovery is not always the same as cold initialization: existing
        # threads and open remote connections must not blindly be started again.
        recover_routine = getattr(self, "recover_routine", None)
        if callable(recover_routine):
            recover_routine()
        elif hasattr(self, "init_control"):
            self.init_control()
        else:
            # As a fallback, we manually activate the known start steps:
            if "S10" in self.marker_register: self.marker_register["S10"].update(True)
            if "S20" in self.marker_register: self.marker_register["S20"].update(True)

        # 3. Switch back to RUNNING mode
        self.mode = PLCMode.RUNNING
