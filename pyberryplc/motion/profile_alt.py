"""
Alternative implementation of motion profiles.

Implements the SCurved profile with a variable top acceleration. If the
acceleration time with acceleration `a_max` becomes smaller than a predefined
lower limit, the acceleration profile during intial/final acceleration of the
motion becomes trapezoidal while the acceleration time remains fixed to its
lower limit.
"""
...
class MotionProfile(ABC):
    ...
    def _calc_v_top_with_time_constraint(self) -> float:
        ...
        v_top_arr = np.linspace(0.0, self.v_max, int(0.5 * self.v_max))
