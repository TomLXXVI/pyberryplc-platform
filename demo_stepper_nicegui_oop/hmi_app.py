...
class StepperHMI(AbstractHMI):
    ...
    def build_ui(self):
        ...
        with ui.row():
            self.btn_left = ui.button(
                "Start left", 
                on_click=lambda: self.shared.hmi_buttons.update({"start_motor_left": True})
            )
            self.btn_right = ui.button(
                "Start right", 
                on_click=lambda: self.shared.hmi_buttons.update({"start_motor_right": True})
            )
            self.btn_stop = ui.button(
                "Stop", 
                on_click=lambda: self.shared.hmi_buttons.update({"stop_motor": True})
            )
            ui.button(
                "Exit HMI", 
    ...
    def update_status(self):
        running = self.shared.hmi_outputs.get("motor_running", False)
